(function(){
    ...
    salida = ()=>{        
        if(lista_datos[0] < lista_datos[1]){
            var suma = 0
            for (let i = lista_datos[0]; i < lista_datos[1]; i++) {
                suma += i
            }
            document.getElementById('salida').innerHTML = "La suma total es: " + suma.toString()
        }else{
            document.getElementById('salida').innerHTML = "Inicio no es menor a fin"
        }
    }
