(function(){
    ...
    // Salida
    salida = ()=>{        
        var edad = lista_datos[0],
            estatura = lista_datos[1]

        if(edad>=13 && edad<=60 && estatura>=160){
            document.getElementById('salida').innerHTML = "El usuario puede ingresar a la atracción."
        }else{
            if(edad<13 || edad>60 && estatura<160){
                document.getElementById('salida').innerHTML = "Edad y estatura no cumplen." 
            }
            else if(edad<13 || edad>60 && estatura>160){
                document.getElementById('salida').innerHTML = "Edad no cumple." 
            }
            else{
                document.getElementById('salida').innerHTML = "Estatura no cumple." 
            }
        }
    }
