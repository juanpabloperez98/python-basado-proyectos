(function(){
    ...
    // Salida
    salida = ()=>{        
        var venta = lista_datos[0],
            descuento = 0,
            precio = 0
        if(venta<500){
            document.getElementById('salida').innerHTML = "No tiene descuento."
        }
        else if(venta>=500 && venta<1000){
            descuento = (venta*5)/100
            precio = venta-descuento
            document.getElementById('salida').innerHTML = "El descuento aplicado es: " + descuento.toString() + ". Precio a pagar: " + precio.toString()
        }
        else if(venta>=1000 && venta<7000){
            descuento = (venta*11)/100
            precio = venta-descuento
            document.getElementById('salida').innerHTML = "El descuento aplicado es: " + descuento.toString() + ". Precio a pagar: " + precio.toString()
        }
        else if(venta>=7000){
